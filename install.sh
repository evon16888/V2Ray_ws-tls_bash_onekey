...
ssl_install() {
    ...
    if [[ "${ID}" == "centos" ]]; then
        ...
        # 针对 Debian/Ubuntu 系统，直接显式指定安装 netcat-openbsd 和 socat
        # 如果系统里只有 netcat-traditional，apt 会自动报错并尝试另一个，但现代系统 100% 支持 openbsd
        echo "正在强制安装 socat 和 netcat-openbsd..."
        apt-get update -y
        apt-get install socat netcat-openbsd -y
    fi
